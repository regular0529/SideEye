...
//
// Joins WiFi "projectbee" (router/STA mode), open http://sideeye-collect.local
// (or the printed IP). CORS enabled on /jpg so a local slideshow page
// (dataset/auto_collect.html) served from localhost can pull frames
// cross-origin for auto-capture, without dropping the PC's internet.
#include <WiFi.h>
#include <WebServer.h>
#include <ESPmDNS.h>
...
const char *MDNS_NAME = "sideeye-collect";
...
<img id="cam" src="/jpg">
<div>
<input id="label" placeholder="라벨 (vehicle 또는 background)" value="vehicle">
